...
const trackedButtonDefaults = {
  "visit-site": "เน€เธเนเธฒเธเธกเธซเธเนเธฒเน€เธงเนเธ",
  register: "เธชเธกเธฑเธเธฃเธชเธกเธฒเธเธดเธ",
  promotion: "เนเธเธฃเนเธกเธเธฑเนเธ",
  "contact-admin": "เธ•เธดเธ”เธ•เนเธญเนเธญเธ”เธกเธดเธ"
};
const directLinkId = "direct";
const directLinkName = "เน€เธเนเธฒเธ•เธฃเธ / เนเธกเนเธกเธตเธฃเธซเธฑเธชเธฅเธดเธเธเน";

const keys = {
  total: "ezpok168:bio:stats:total",
  updatedAt: "ezpok168:bio:stats:updatedAt",
  counts: "ezpok168:bio:stats:counts",
  lastClicks: "ezpok168:bio:stats:lastClicks",
  events: "ezpok168:bio:stats:events",
  links: "ezpok168:bio:stats:links",
  linkCounts: "ezpok168:bio:stats:linkCounts",
  linkLastClicks: "ezpok168:bio:stats:linkLastClicks",
  linkButtonCounts: "ezpok168:bio:stats:linkButtonCounts"
};
...
function parseLinkRecords(rawLinks, counts, lastClicks, buttonCounts) {
...
async function getStats() {
  const [total, updatedAt, rawCounts, rawLastClicks, rawEvents, rawLinks, rawLinkCounts, rawLinkLastClicks, rawLinkButtonCounts] = await redisPipeline([
    ["GET", keys.total],
    ["GET", keys.updatedAt],
    ["HGETALL", keys.counts],
  ...
  const links = parseLinkRecords(rawLinks, rawLinkCounts, rawLinkLastClicks, rawLinkButtonCounts);
...
async function createBioLink(input, req) {
  const name = cleanText(input.name || input.platform || input.label, "เธฅเธดเธเธเน Bio");
...
async function resetStats() {
  await redisCommand(["DEL", keys.total, keys.updatedAt, keys.counts, keys.lastClicks, keys.events, keys.linkCounts, keys.linkLastClicks, keys.linkButtonCounts]);
  return { ok: true };
}
